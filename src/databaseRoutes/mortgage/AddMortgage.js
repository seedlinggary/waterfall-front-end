...
const AddMortgage = ({deal,company,diligence_id}) => {
    ...
    const addMortgage = e => {
      ...
      const SendApi = (e) => {
        //   e.preventDefault();
          let info = mortgages
          console.log(mortgages)
          let a = apiRequest('POST',info,`/mortgage/${company.id}/${deal.id}/${diligence_id}`)
          console.log(a)
        //   navigate(0)
        } 
      ...
      function edit_times(data) {
            ...
            let newList = data.map((item, i) => 
            {
              ...
              return {...item,  'start_date': new_date}; // else return unmodified item 
            });
 ...
 <Accordion defaultActiveKey="0" flush>
      ...
      <Accordion.Item eventKey={i}>
        <Accordion.Header>
                        <h3>Name: {mortgages[i].name} / Amount:{mortgages[i].amount} </h3>
                        <span className="AccordionButton" onClick={deleteMortgage}>  Delete me</span> 
