...
const ActiveDealHome = () => {
    ...
    const mystyle = {
      ...
      const { data: investors, err, isPen} = useFetch(`/investor/deal_investors/${company.id}/${deal.id}` , requestOptions)
        ...
        {properties && properties.map((property, i) => {
            return (
              ...
              { property.purchased &&
                ...
                <Accordion.Body>
                <Link to={`/PropertyHome/`} state={{'company': company, 'deal':deal, 'property':property}}>
 
                <h5>See Property Info: {property.street}</h5>
                </Link>
                <Button
            ...
            <AddBill pAndLs={pAndLs} setPandLs={setPandLs} expenseTypes={expenseTypes} deal={deal} company={company} diligence_id={diligence_id} bill_url={'Deal'} url_id={deal.id}/>

              Coming Soon</Tab.Pane>
            <Tab.Pane eventKey="fourth">
              <MortgageHome deal={deal} company={company} diligence_id={diligence_id} bill_url={'Deal'} url_id={deal.id} />
              <MultiTree deal={deal} company={company} diligence_id={diligence_id} active_tracking={true}/>
