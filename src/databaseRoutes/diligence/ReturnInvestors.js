...
function ReturnInvestors({pId, setParentId,tree, setTree,waterfall, setWaterfall,lp, setLP, gp, setGP,payoutFrequency, setPayoutFrequency, investmentDetails}) {
  ...
  // function ShowInvestmentDetails(investmentdetails){
  // investmentdetails.map( (info,index) => {
  //     return(
  //     <div><h3> contribution per year {info.contributions}</h3></div>
...
<Tab eventKey={index} title={index}>
    ...
    const renderPerson = (person) => {
      ...
      justify
    >
{ investmentDetails && investmentDetails[person.id] && investmentDetails[person.id].map( (info,index) => {
return(
<div><h3> Year {info.contributions}</h3></div>
 ...
 */}
 ...
 { investmentDetails && investmentDetails[person.id] && ShowInvestmentDetails(investmentDetails[person.id])
}
